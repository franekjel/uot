...
void rendering::render_selected_sector_info(const client_context& context)
{
    ...
    sdl_utilities::render_text(r.get(), gr->main_font,
    ...
    render_planet_helper(context, 2.0, size_settings::context_area::width / 2,
                         std::min(250, planets_meta::texture_size[sector_id] * 2),
                         gr->planetTextures[gui->galaxy_stars_textures.at(gui->current_sector.value()->sector_id)]);

    // render planet info
    if (!gui->cur_sector_info.has_value())
    {
        sdl_utilities::render_text(r.get(), gr->secondary_font, " loading...", size_settings::context_area::width / 2,
    ...
    sdl_utilities::render_text(r.get(), gr->secondary_font, planet_info + resource_info + fleet_info,
