...
std::shared_ptr<SDL_Texture> sdl_utilities::load_texture_from_file(const std::string& path, const std::shared_ptr<SDL_Renderer>& r) {
    SDL_Surface* loadedSurface = IMG_Load(path.c_str());
    if(loadedSurface == NULL) {
    	throw std::runtime_error( "Unable to load image %s! SDL_image Error: %s\n" + path + + " " + std::string(IMG_GetError() ));
    } 

    SDL_SetColorKey( loadedSurface, SDL_TRUE, SDL_MapRGB(loadedSurface->format, 0, 0xFF, 0xFF));
    const auto ret = std::shared_ptr<SDL_Texture>(SDL_CreateTextureFromSurface( r.get(), loadedSurface ), sdl_texture_deleter);
    if(ret.get() == NULL) {
...
std::shared_ptr<SDL_Window> sdl_utilities::sdl_create_window(const std::string title, int x, int y, int w, int h, Uint32 flags) {
    const auto ret = std::shared_ptr<SDL_Window>(SDL_CreateWindow(title.c_str(), x, 
            y, w, h, flags), sdl_window_deleter);
...
std::shared_ptr<SDL_Renderer> sdl_utilities::sdl_create_renderer(const std::shared_ptr<SDL_Window>& w, int index, Uint32 flags) {
